...
> 默认连接：`localhost:3306` / `root` / `123456` / `BookDB`  
> 修改连接信息：编辑 `DBHelper.cs` 中的 `ConnStr` 常量
...
完整课程设计报告（DOCX格式）已包含在本仓库中。

---

> 📘 本项目为 C# 程序设计课程综合项目  
> 原要求：SQL Server → 实际使用：MySQL 8.0  
> 原要求：Visual Studio → 实际使用：csc.exe 命令行编译
